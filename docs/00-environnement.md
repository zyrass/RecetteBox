...
> Objectif : disposer d'un poste Windows 11 capable de compiler, exécuter et servir une application Laravel 13 + Livewire 4 + Tailwind 4, sans Docker ni WSL, avec une empreinte mémoire minimale.
...
```mermaid
flowchart TD
    Start([Windows 11 brut]) --> Git[Étape 1<br/>Installer Git for Windows]
    Git --> Node[Étape 2<br/>Installer Node.js 22 LTS]
    Node --> PhpNew[Étape 3<br/>php.new<br/>PHP 8.4 + Composer + Laravel]
    PhpNew --> Sqlite[Étape 4<br/>Vérifier SQLite]
    Sqlite --> VsCode[Étape 5<br/>VS Code + extensions]
    VsCode --> Repo[Étape 6<br/>git init recettebox]
    Repo --> Check{Vérifications<br/>OK ?}
    Check -- Oui --> Done([Environnement prêt<br/>Passage Phase 1])
    Check -- Non --> Debug[Voir section<br/>Pièges courants]
    Debug --> Check
...
> **Note.** On ne pousse rien sur GitHub à ce stade : ce sera fait à la fin de la Phase 1, quand le code Laravel existe.
